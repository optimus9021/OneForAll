...
def menu_utama():
    while True:
        ...
        if pilihan == '1':
            from Script.routes import get
            clear_screen()
            get.get()
        elif pilihan == '2':
            from Script.routes import process
            clear_screen()
            process.process()
        elif pilihan == '3':
            from Script.routes import training
            clear_screen()
            training.training()
        elif pilihan == '4':
            from Script.routes import validasi
            clear_screen()
            validasi.validasi()
        elif pilihan == '5':
            from Script.routes import prediksi
            clear_screen()
            prediksi.prediksi()
        elif pilihan == '6':
            clear_screen()
